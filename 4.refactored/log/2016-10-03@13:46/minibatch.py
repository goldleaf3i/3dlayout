...
def start_main(parametri_obj, path_obj):	
	...
	if nuovo_clustering == 1:
		...
		for segmento in walls:
			if segmento.cluster_muro ==-1:
				outliers.append(segmento)
		
		walls_senza_outliers = []
		cluster_mura_senza_outliers = []
		for segmento, cluster in zip(walls, cluster_mura):
			if segmento.cluster_muro !=-1:
				walls_senza_outliers.append(segmento)
				cluster_mura_senza_outliers.append(cluster)
		
		print cluster_mura_senza_outliers
		exit()
