	...
	if par.azione_complessa == True:
		...
		for index,s in enumerate(spazi_parziali):
			...
			for indice,permutazione in enumerate(permutazioni):
				...
				print "fatto 1"
				try:
					dsg.disegna_spazi(plan_o.spazi, dsg.get_colors(plan_o.spazi), xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = 'permutazioni_stanza'+str(index)+'\14_permutazioni_'+str(indice)+'_prima', format='png')
				...
				print "fatto 3"
				try:
					dsg.disegna_spazi(plan_o.spazi, dsg.get_colors(plan_o.spazi), xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = 'permutazioni_stanza'+str(index)+'\14_permutazioni_'+str(indice)+'_dopo', format='png')
				except AssertionError:
					pass
				print "fatto 4"
