...
# 		print len(set(cluster))
	...
	#cerco le celle parziali(da completare, non funziona)
	celle_parziali, parz_poligoni = lay.get_celle_parziali(celle, celle_out)
	
	#cancellare
	print len(celle_parziali)
	print len(parz_poligoni)
	label = range(0,len(parz_poligoni))
	colori2 = dsg.get_colors(label)
	dsg.disegna_stanze(parz_poligoni, colori2, xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = "ciao")
