...
# 			dsg.disegna_stanze(poligoni,colori , xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = '15_poligoni_esterni_stanza'+str(index), format='png')	
	...
	if par.mappa_completa:
		...
		if par.DISEGNA:
			...
			for dizionario in lista_dizionari_configurazione:
				chiavi = dizionario.keys()
				f = open(path_obj.filepath+'accuracy_imm/9_accuracy_'+str(i)+'.txt','a')
				f.write('------------------------\n informazioni stanza parziale per stanza parziale\n')
				for chiave in chiavi:
					f.write('id_della_stanza_XML_GT = '+str(chiave)+'\n')
					f.write('id_della_stanza_XML_predizione = '+str(dizionario[chiave])+'\n')
