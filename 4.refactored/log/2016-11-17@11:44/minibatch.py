	...
	if par.azione_complessa == True:
		...
		for index,s in enumerate(spazi_parziali):
			...
			for indice,permutazione in enumerate(permutazioni):
				...
				#calcolo penalita'
				penal1_dopo = val.penalita1(stanza)#piu' questo valore e' alto peggio e', valori prossimi allo zero indicano frome convesse.
				penal4_dopo = val.penalita4(stanza, plan_o, celle_di_altre_stanze)#conto il numero di extended che ci sono dopo aver aggiungere la permutazione, sfavorisce i gradini
				...
				if type(s.spazio)== Polygon:
					...
					score = val.score_function(penal1_dopo, penal2_dopo)#non ancora implementata fino alla fine
					score_permutazioni_corrette.append(score)
				
					#----------------------fine valutazione-----------------------------------
					
					#disegno
					dsg.disegna_spazi(plan_o.spazi, dsg.get_colors(plan_o.spazi), xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = 'permutazioni/14_stanza'+str(index)+'permutazioni_'+str(indice)+'_a', format='png')
				else:
				...
				if ok ==True:
					dsg.disegna_spazi(plan_o.spazi, dsg.get_colors(plan_o.spazi), xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = 'permutazioni/14_stanza'+str(index)+'permutazioni_'+str(indice)+'_b', format='png')
				#------	
			...
			penal1_prima = val.penalita1(stanza)#piu' questo valore e' alto peggio e', valori prossimi allo zero indicano frome convesse.	
			penal4_prima = val.penalita4(stanza, plan_o, celle_di_altre_stanze)#conto il numero di extended che ci sono prima di aggiungere la permutazione
			score_originale = val.score_function(penal1_prima, penal2_prima)#non ancora implementata fino alla fine
			...
			if min_score>=score_originale:
				...
				#il layout originale e' migliore di tutti gli altri, non faccio nessuana previsione
				pass
