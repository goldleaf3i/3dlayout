	...
	if par.azione_complessa == True:
		...
		for index,s in enumerate(spazi_parziali):
			...
			for indice,permutazione in enumerate(permutazioni):
				pgeom.aggiunge_celle_permutazione(permutazione, plan_o, stanza)
				dsg.disegna_spazi(plan_o.spazi, dsg.get_colors(plan_o.spazi), xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = '14_permutazione'+str(indice), format='png')	
