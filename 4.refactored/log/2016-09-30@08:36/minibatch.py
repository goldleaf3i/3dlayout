...
def start_main(parametri_obj, path_obj):	
	...
	if nuovo_clustering == 1:
		...
		#ora che ho un insieme di cluster voglio andare ad unire quelli molto vicini
		...
		#ora voglio andare a classificare ulteriormente i rappresentanti
		#qui va settata la soglia con cui voglio separare i cluster
		#TODO: devi inserire in parametri.py la soglia con cui vuoi separare i vari cluster
		segmenti_rappresentanti = sg.spatialClustering(10, segmenti_rappresentanti)
		...
		print len(segmenti_rappresentanti)
		...
		#controllo un attimo di no aver settato il cluster_spaziale
		print len(walls)
		print len(segmenti_rappresentanti)
		print len(set(cluster_mura))
		print len(cluster_mura)

		
	elif nuovo_clustering ==3:
		#TODO: questo pezzo e' da eliminare non serve piu.
		cluster_spaziali = lay.get_new_cluster_spaziale(walls, cluster_angolari, parametri_obj)
	
	print cluster_spaziali
	print len(set(cluster_spaziali))
	print len(cluster_angolari)	
	print len(set(cluster_angolari))
	#exit()#prova ad usare cluster angolare
