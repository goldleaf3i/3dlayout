		...
		if par.AZIONE == "batch":
			...
			for DATASET in path_obj.DATASETs :
				...
				for metricMap in glob.glob(path_obj.INFOLDERS+'IMGs/'+DATASET+'/*.png') :
					...
					parametri_obj.minLateralSeparation = 7 if (DATASET=='SCHOOL' or DATASET=='PARZIALI') else 15	
					#parametri_obj.cv2thresh = 150 if DATASET == 'SCHOOL' else 200
					parametri_obj.cv2thresh = 170 if (DATASET=='SCHOOL' or DATASET=='PARZIALI') else 200
