...
# 		
	...
	if separaStanzeUndersegmentate ==1:
		...
		dsg.disegna_spazi(plan_o.spazi, dsg.get_colors(plan_o.spazi), xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = '13c_UNDERSEGMETAZIONE', format='png')
...
# 				for i in e:
# 					prova_edges.append(i)
# 		
# 		dsg.disegna_dbscan(clustersCelle, prova_celle, prova_celle_poligoni, xmin, ymin, xmax, ymax, prova_edges, contours,filepath = path_obj.filepath, format='png', savename = '6b_DBSCAN_segmento')
# 		
...
# 			dsg.disegna_stanze(poligoni,colori , xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = '15_poligoni_esterni_stanza'+str(index), format='png')	
	...
	if par.DISEGNA:
		#sto disegnando usando la lista di colori originale, se voglio la lista della stessa lunghezza sostituire colori con colori_reali
		dsg.disegna_spazi(plan_o.spazi, dsg.get_colors(plan_o.spazi), xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = '8_spazi_reali', format='png')
		#dsg.disegna_stanze(stanze_reali, colori_reali, xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = '8_Stanze_reali', format='png')	
	#------------------------------------------------------------------------------------
	if par.DISEGNA:
		dsg.disegna_distance_transform(distanceMap, filepath = path_obj.filepath, format='png')
		dsg.disegna_medial_axis(points, b3, filepath = path_obj.filepath, format='png')
		dsg.plot_nodi_e_stanze(dsg.get_colors(stanze),estremi, G, pos, stanze, stanze_collegate, filepath = path_obj.filepath, format='png')
