...
# 		
	...
	if par.MCMC ==True:
		...
		for index,s in enumerate(spazi_parziali):
			...
			#divido il ricerca delle extended in due parti, nella prima cerco gli extended in comune con le celle esterne,
			# nella seconda invece estraggo quelli confinanti con le celle di altre stanze				 
		
			a = sp.estrai_extended_da_spazio(s, plan_o.extended_segments, celle_di_altre_stanze)
			a2 = sp.estrai_extended_da_spazio(s, plan_o.extended_segments, cellette_out)
			tot_segment = list(set(a+a2))
			...
			tot_segment_visti = list(set(b+c))
			...
			mcmc.computa_MCMC(s, plan_o_3) #faccio Markov chain monte carlo sulla copia 3 di plan_o
		...
		#prova elimina cella
		sp.elimina_cella_da_spazio(spp, cella_prova, plan_o_2)
		dsg.disegna_spazi(plan_o_2.spazi, dsg.get_colors(plan_o_2.spazi), xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = '13d_azione_geom_2_meno_ultima_cella', format='png')	
