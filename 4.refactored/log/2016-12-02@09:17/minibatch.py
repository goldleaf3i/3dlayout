...
def start_main(parametri_obj, path_obj):
	...
	start_time_cluster_seg = time.time()
...
# 		dsg.disegna_segmenti(outliers, savename = "outliers")
	...
	end_time_cluster_seg = time.time()	
	print "il clustering dei segmenti impiega ", end_time_cluster_seg-start_time_cluster_seg
...
# 		
	...
	#in questo punto devi inserire il TODO:(1*) che togli le stanze non collegate
	...
	'''
	TODO:(1*) questo pezzo va inserito quando inserisci la parte sul grafo topologico che devi inserire alla fine
...
# 			dsg.disegna_stanze(poligoni,colori , xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = '15_poligoni_esterni_stanza'+str(index), format='png')	
	...
	#riottengo tutti i dati che i  servono (dato che ho cambiato la struttura dati)
	stanze = []
	for s in plan_o.spazi:
	...
	print "il tempo necessario al main e'", end_time_main -start_time_main
	print "per trovare le frontiere ci vuole", end_time_frontiere- start_time_frontiere
	print "del tempo necessario al main, l'azione complessa occupa:", end_time_azione_complessa-start_time_azione_complessa
	print "del tempo necessario per l'azione geometrica 1 invece occupa", end_time_azione_1 -start_time_azione_1
	print "in particolare l'azione complessa per ogn stanza impiega"
	...
	for e,dt,dr,t in zip(lista_n_elementi, lista_n_disposizioni_totali, lista_n_disposizioni_reali, lista_tempi):
		print "la stanza ", indice_s
		print "il numero di elementi dell'insieme delle celle confinanti e':", e
		print "il numero di dispsizioni totali ottenuti sono: ", dt
		print "il nuemro di disposizioni legali sono: ", dr
		print "il tempo impiegato per processare questa stanza e': ", t
		print "---------------------------------"
		indice_s +=1
	return results
	#TODO
