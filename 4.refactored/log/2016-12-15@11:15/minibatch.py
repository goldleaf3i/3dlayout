...
# 			dsg.disegna_stanze(poligoni,colori , xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = '15_poligoni_esterni_stanza'+str(index), format='png')	
	...
	if par.mappa_completa:
		...
		stanze_gt = acpar.get_stanze_gt(path_obj.nome_gt) #prelevo tutte le stanze gt dall'xml
		...
		#calcolo delle corrispondenze per tutte le mappe
		#...
		
		#calcolo percentuali di completamento della mappa 
		percentuali_completamento_mappa = [] #in questa lista ci sono tutte le percentuali di completamento della mappa prendendo in considerazione tutte le configurazioni in lista_gt_proporzionata
		lista_percentuali_completamento_mappa_bwd, lista_percentuali_completamento_mappa_fwd, lista_percentuali_completamento_GT_bwd, lista_percentuali_completamento_GT_fwd= acpar.calcola_percentuali_completamento_mappa(stanze, lista_gt_proporzionata)
		i = 0
		for perc_mappa_bwd, perc_mappa_fwd, perc_GT_bwd, perc_GT_fwd in zip(lista_percentuali_completamento_mappa_bwd, lista_percentuali_completamento_mappa_fwd, lista_percentuali_completamento_GT_bwd, lista_percentuali_completamento_GT_fwd):
			print "n_configurazione, perc_mappa_bwd, perc_mappa_fwd, perc_GT_bwd, perc_GT_fwd ---> ", i, perc_mappa_bwd, perc_mappa_fwd, perc_GT_bwd, perc_GT_fwd
			i+=1
		...
		if par.DISEGNA:
			...
			#dsg.disegna_grafici_per_accuracy(stanze_simili, stanze_gt_simili, filepath = path_obj.filepath, format='png', savename ='20_similarita')
			SAVE_FOLDER = os.path.join(path_obj.filepath, 'accuracy_imm')
			if not os.path.exists(SAVE_FOLDER):
				os.mkdir(SAVE_FOLDER)
			for index,gt_prorp in enumerate(lista_gt_proporzionata):
				dsg.disegna_grafici_per_accuracy(stanze_acc, gt_prorp, filepath = path_obj.filepath, format='png', savename = 'accuracy_imm/9_accuracy_'+str(index))
