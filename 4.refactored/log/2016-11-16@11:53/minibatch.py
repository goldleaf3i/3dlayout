...
# 		
	...
	if par.azione_complessa == True:
		...
		for index,s in enumerate(spazi_parziali):
			...
			level= 3 #questa e la profondita con la quale faccio la mia ricerca, oltre al terzo livello non vado a ricercare le celle.
			
			elementi = pgeom.estrai_spazio_delle_celle(s, plan_o, level)
			poligoni= []
			for ele in elementi:
				poligoni.append(ele.cella)
			dsg.disegna_stanze(poligoni, dsg.get_colors(poligoni), xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = '15_poligoni_esterni_stanza'+str(index), format='png')	

		
			
		
	if par.azione_semplici==True:
