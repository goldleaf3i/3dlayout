...
# 		dsg.disegna_segmenti(outliers, savename = "outliers")
	...
	trova_parziali=2 #se voglio il metodo che controlla le celle metto 1, se voglio il confronto di un intera stanza con l'esterno metto 2
	...
	uniciStanzeOversegmentate = 3
	#1) primo controlla cella per cella 
	#2) unisce facendo una media pesata
	#3) non unisce le celle, usato per mappe parziali
	if uniciStanzeOversegmentate ==1:
		...
		dsg.disegna_spazi(plan_o.spazi, dsg.get_colors(plan_o.spazi), xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = '13b_spazi_nuovi')	
	elif uniciStanzeOversegmentate == 2:
		#TODO: questo metodo funziona meglio del primo, vedere se vale la pena cancellarlo
		#metodo molto simile a quello di Mura per il postprocessing
		print len(plan_o.spazi)
		
		plan.postprocessing(plan_o, parametri_obj)
		
		c=0
		for s in plan_o.spazi:
			if s.spazio.geom_type == 'Polygon':
				c+=1
		print c
		dsg.disegna_spazi(plan_o.spazi, dsg.get_colors(plan_o.spazi), xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = '13b_spazi_nuovi')	
	else:
		pass #se non voglio unire le stanze, ad esempio e' utile quando sto guardando le mappe parziali
