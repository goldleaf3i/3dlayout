...
# 		
	...
	if par.azione_complessa == True:
		...
		for index,s in enumerate(spazi_parziali):
			...
			per = pgeom.possibili_permutazioni(elementi)
			#elimino permutazioni vuote
			permutazioni=[]
			for p in per:
				if len(p)>0:
					permutazioni.append(p)
			...
			for indice,permutazione in enumerate(permutazioni):
				...
				pgeom.aggiungi_celle(permutazione, plan_o, s)
				print "fatto 1"
				dsg.disegna_spazi(plan_o.spazi, dsg.get_colors(plan_o.spazi), xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = '14_permutazione_prima'+str(indice), format='png')	
				print "fatto 2"
				#pgeom.elimina_celle_permutazione(permutazione, plan_o, s)
				pgeom.elimina_celle(permutazione, plan_o, s)
				print "fatto 3"
				dsg.disegna_spazi(plan_o.spazi, dsg.get_colors(plan_o.spazi), xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = '14_permutazione_dopo'+str(indice), format='png')	
				print "fatto 4"
