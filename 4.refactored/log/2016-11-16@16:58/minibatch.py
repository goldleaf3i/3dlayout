	...
	if par.azione_complessa == True:
		...
		for index,s in enumerate(spazi_parziali):
			...
			for indice,permutazione in enumerate(permutazioni):
				errore = pgeom.aggiunge_celle_permutazione(permutazione, plan_o, s)
				print errore
				print "fatto 1"
				if errore==False:
					try:
						dsg.disegna_spazi(plan_o.spazi, dsg.get_colors(plan_o.spazi), xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = 'permutazioni_stanza'+str(index)+'\14_permutazione_'+str(indice)+'_prima', format='png')
					except AssertionError:
						pass
				print "fatto 2"
				errore = pgeom.elimina_celle_permutazione(permutazione, plan_o, s)
				print errore
				print "fatto 3"
				if errore== False:	
					try:
						dsg.disegna_spazi(plan_o.spazi, dsg.get_colors(plan_o.spazi), xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = 'permutazioni_stanza'+str(index)+'\14_permutazione_'+str(indice)+'_dopo', format='png')
					except AssertionError:
						pass
				print "fatto 4"
