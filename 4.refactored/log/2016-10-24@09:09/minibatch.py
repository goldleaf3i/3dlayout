...
# 		dsg.disegna_segmenti(outliers, savename = "outliers")
	...
	if par.mappa_completa == False and trova_parziali==3:
		...
		if par.DISEGNA:
			dsg.disegna_map(immagine_cluster,filepath = path_obj.filepath, savename = '0a_frontiere')
	...
	if uniciStanzeOversegmentate ==1:
		...
		dsg.disegna_spazi(plan_o.spazi, dsg.get_colors(plan_o.spazi), xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = '13b_spazi_nuovo')	
	...
	#AZIONE GEOMETRICA 1) AGGIUNGO CELLE OUT A CELLE PARZIALI SOLO BASANDOMI SUL PESO DEGLI EXTENDED SEGMENT
	
	#provo ad unire una cella
	...
	for s in spazi_parziali:
		celle_confinanti = pgeom.ricerca_cella_da_unire_a_spazio_1(plan_o, s, extended_segments )
		print "le celle confinanti sono: ", len(celle_confinanti)
		#prova stampa
		st = []
		for index, c in enumerate(celle_confinanti):	
			st.append(c.cella)
		dsg.disegna_stanze(st, colori, xmin, ymin, xmax, ymax,savename='8b_celle_da_unire'+str(index))
		
	
		
# 		
# 		#faccio una prova per unire una cella.
...
# 				
# 	dsg.disegna_spazi(plan_o.spazi, dsg.get_colors(plan_o.spazi), xmin, ymin, xmax, ymax,filepath = path_obj.filepath, savename = '13c_spazi_nuovo_con_celle_aggiunte')	
# 
# 		
	
	
	#prova 
	#per ogni cella parziale aggiungo una cella in base al peso degli extended lines
	# for s in spazi_parziali:
# 		for co in plan_o.cellette_esterne:	
# 			pgeom.add_cella_a_spazio_1(co, s, plan_o.extended_segments)	
# 			
