...
# 		
	...
	for index, e in enumerate(extended_segments):
		print "index: ", index, e.copertura_stanze
	...
	#-----AGGIUNGO CELLE OUT A CELLE PARZIALI SOLO BASANDOMI SUL PESO DEGLI EXTENDED SEGMENT
	...
	for s in spazi_parziali:
		...
		#ora che ho tutte le celle confinanti con una parte parziale di una stanza, posso andare a selezionare 
		#le celle da unire in base a:
		
		#1)copertura che hanno gli edge sugli extended segments
		#se una cella ha almeno un bordo (che non tocca la cella interna parziale) il cui extended segment corrispondente e' molto pesante, allora la seleziono
