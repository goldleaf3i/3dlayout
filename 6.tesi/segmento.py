...
def lunghezza(x1,y1,x2,y2):
	'''
	calcola e ritorna distanza tra punto 1 e punto 2
	'''
...
def segments_intersect(x11, y11, x12, y12, x21, y21, x22, y22):
	  """ ritorna true se i segmenti si intersecano, false altrimenti
...
def segments_distance(x11, y11, x12, y12, x21, y21, x22, y22):
	""" calcola distanza tra 2 segmenti, se si intersecano return 0, altrimenti la calcola e la ritorna
...
def disegna_segmenti(lista):
	...
	for s in lista:
		...
		plt.plot(ascisse,ordinate, color='k',linewidth=2)
		del ascisse[:]
		del ordinate[:]
	plt.show()
...
'''
def set_cluster_spaziali(soglia, lista_muri):
	...
	for index,muro1 in enumerate(lista_muri): 
		if (muro1.cluster_spaziale == None): #se e' gia stato settato
		...
		else: #gia' stato settato, ma devo controllare lo stesso i successivi perche' possono essercene alcuni che dovrebbero
...
def spatialClustering(soglia, lista_muri):
	...
	for index,muro1 in enumerate(lista_muri):
		...
		#disegna_cluster_spaziali(cluster_spaziali, lista_muri)
	return lista_muri
...
def lateral_separation(muro1,muro2):
	...
	if(m!=0): #se il cluster_angolare non e' 0, quindi retta non orizzontale
		...
		#il mid2 proiettato avra' x = mid1_x e y = mid2_y
		return lunghezza(mid1_x,mid1_y,mid1_x,mid2_y)


def disegna_cluster_angolari(cluster_centers, lista_muri, cluster_angolari):
	'''
	disegna con lo stesso colore i muri con stesso cluster angolare
	''' 
	ascisse = []
	ordinate = []
	plt.title('cluster angolari')
	...
	for c in set(cluster_angolari):
		for muro1 in lista_muri:
			if muro1.cluster_angolare == c:
				...
				plt.plot(ascisse,ordinate, color='r', linewidth=2.0)
				del ascisse[:]
				del ordinate[:]
			else:
				ascisse.extend((muro1.x1,muro1.x2))
				ordinate.extend((muro1.y1,muro1.y2))
				plt.plot(ascisse,ordinate, color='g', linewidth=2.0)
				del ascisse[:]
				del ordinate[:]
		plt.show()


def disegna_cluster_spaziali(cluster_spaziali, lista_muri):
	'''
	...
	for index,c in enumerate(np.random.permutation(list(set(cluster_spaziali)))):
		for muro in lista_muri:
			if muro.cluster_spaziale == c:
				...
				plt.plot(ascisse,ordinate, color=colors.rgb2hex(scalarMap.to_rgba(index)), linewidth=2.0)
				del ascisse[:]
				del ordinate[:]
	plt.show()
...
def intersezione(x1,y1,x2,y2,x3,y3,x4,y4):
	'''
	dati gli estremi di 2 segmenti ritorna punto di intersezione 
	'''
	if(x1!=x2)and(x3!=x4):
		m1 = (y2-y1)/(x2-x1)
		m2 = (y4-y3)/(x4-x3)
...
def gia_dentro_seg(seg, proiezioni):
	'''
	ritorna true se il segmento e' gia' nelle proiezioni
	'''
...
def compreso(seg1, lista):
	'''
	ritorna true se il segmento e' interamente compreso in un altro segmento della lista
	'''
	x1 = seg1[0]
	y1 = seg1[1]
	x2 = seg1[2]
...
def unisci_sovrapposti(proiezioni):
	'''
	ritorna 1 ogni volta che vengono trovate 2 proiezioni che si sovrappongono parzialmente, e viene creata e aggiunta l'unione delle 2 		proiezioni, e queste 2 proiezioni vengono eliminate dalla lista delle proiezioni.
	'''
...
def calcola_cov(edge, proiezioni):
	'''
	ritorna la somma delle lunghezze delle proiezioni sull'edge, in pratica ritorna lo spazio di edge occupato dalle proiezioni
	'''
	x1 = edge.x1
	y1 = edge.y1
	x2 = edge.x2
...
def edge_adiacenti(x, y, edge, edges):
	'''
	ritorna lista di edges diversi dall'edge, con m diversa, e che come estremo hanno x1 y1
	'''
	e = []
...
def stessa_m(edge, altro_edge):
	'''
	ritorna true se i 2 edge hanno stessa inclinazione
	'''
	x1 = edge.x1
	y1 = edge.y1
	x2 = edge.x2
