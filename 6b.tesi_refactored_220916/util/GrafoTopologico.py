...
def get_grafo(metricMap, stanze, estremi, colori, filepath = '.'):
	'''
	...
	#---- ottavo/nono plot ---> 8:all'interno di questa funzione disegno la mappa di distanza;9: disegno anche i medial axis (TODO capire cosa sono)
	medialAxis = mdl.medial_points(im_bw, filepath=filepath)
	...
	#----------------------------------GRAFO---------------------------------------------
	G, pos = crea_grafo(stanze, stanze_collegate, estremi, colori, filepath = filepath)
	#------------------------------------------------------------------------------------
	return G, pos, stanze_collegate, porte_colleganti
	
def  crea_grafo(stanze, stanze_collegate, estremi, colori, filepath = '.'):
...
# 	plt.show() #visualizzo il grafo topologico.	
	...
	G,pos = dsg.plot_nodi_e_stanze(colori,xmin, xmax, ymin, ymax, G, pos, stanze, stanze_collegate, filepath = filepath)
