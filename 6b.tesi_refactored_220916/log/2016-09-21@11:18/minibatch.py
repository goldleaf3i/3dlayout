...
def load_main(layoutPath, grafoTopologicoPath):
	#carico layout
	pkl_file = open( layoutPath, 'rb')
	...
	pkl_file2 = open( grafoTopologicoPath, 'rb')
	...
	print "ho caricato tutto quello che dovevo caricare ora cerco di creare il file xml dei parametri"
	par.to_XML(parameter_obj)
			...
			for DATASET in path_obj.DATASETs :
				...
				for metricMap in glob.glob(path_obj.INFOLDERS+'IMGs/'+DATASET+'/*.png') :
					...
					if par.LOADMAIN==False:
						...
						load_main(path_obj.filepath_pickle_layout, path_obj.filepath_pickle_grafoTopologico)
						LOGFILE.flush()
