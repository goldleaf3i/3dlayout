...
def to_XML(parameter_obj, path_obj):
	...
	ET.SubElement(par, "p5", name="rho").text = str(parameter_obj.minVal)
	...
	ET.SubElement(par, "p8", name="minLineLength").text = str(parameter_obj.thresholdHough)	
	ET.SubElement(par, "p9", name="maxLineGap").text = str(parameter_obj.maxLineGap)
	ET.SubElement(par, "p10", name="eps").text = str(parameter_obj.maxLineGap)
...
def load_from_XML(parXML):
	...
	for parametro in root.findall('parametri'):
		...
		parameter_obj.theta = int(parametro.find('p6').text)
		...
		parameter_obj.eps = int(parametro.find('p10').text)
		parameter_obj.minPts = int(parametro.find('p11').text)
		parameter_obj.h = int(parametro.find('p12').text)
		parameter_obj.minOffset = int(parametro.find('p13').text)
		parameter_obj.diagonali = int(parametro.find('p14').text)
		parameter_obj.m = int(parametro.find('p15').text)
		parameter_obj.flip_dataset = int(parametro.find('p16').text)
