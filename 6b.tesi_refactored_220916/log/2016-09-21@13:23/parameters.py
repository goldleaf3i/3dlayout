...
def load_from_XML(parXML):
	...
	for path in root.findall('path'):
		path_obj.INFOLDERS = parametro.find('p1').text
		path_obj.OUTFOLDERS = parametro.find('p2').text
		#path_obj.DATASETs = parametro.find('p3').text  #TODOquesto non e' una stringa ma una lista non puoi fare in qusto modo, ma siccome non ti dovrebbe servire mai per ora non lo caricare la lascialo vuoto
		path_obj.data_out_path = parametro.find('p4').text
		path_obj.metricMap = parametro.find('p5').text
		path_obj.nome_gt = parametro.find('p6').text
		path_obj.filepath = parametro.find('p7').text
		path_obj.filepath_pickle_layout = parametro.find('p8').text
		path_obj.filepath_pickle_grafoTopologico = parametro.find('p9').text
